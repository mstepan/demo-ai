...
public class ChatService {
    ...
    /**
     * Handles non-streaming chat requests. Metrics (Prometheus/Micrometer): -
     ...
     * app_oci_chat_failures_total (Counter): increments on failures (exceptions or invalid
     * responses) Notes: - Keep metric tag values low-cardinality to avoid cardinality explosions.
     */
    ...
    public Answer askQuestion(Question question) {
        ...
        try {
            ...
            if (chatResponse == null) {
                meterRegistry
                        .counter(RegisteredMetrics.API_OCI_CHAT_TOTAL, "status", "failed")
                        .increment();
            ...
            meterRegistry
                    .counter(RegisteredMetrics.API_OCI_CHAT_TOTAL, "status", "success")
                    .increment();
            ...
            sampleTimer.stop(
                    Timer.builder(RegisteredMetrics.API_OCI_CHAT_LATENCY_SECONDS)
    ...
    public Answer recover(AnswerNotRelevantException ex) {
        ...
        meterRegistry
                .counter(
                        RegisteredMetrics.APP_LLM_RETRIES_TOTAL,
                        "reason",
                        "AnswerNotRelevantException")
                .increment();
    ...
    private void evaluateRelevancy(String questionText, String answerText) {
        ...
        meterRegistry
                .counter(
                        RegisteredMetrics.APP_EVALUATOR_RELEVANCE_TOTAL,
                        "outcome",
                        pass ? "yes" : "no")
                .increment();
        if (!pass) {
