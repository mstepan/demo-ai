...
public class OCIGenAiStreamingSinkFactory {
    ...
    public java.util.function.Consumer<reactor.core.publisher.FluxSink<org.springframework.ai.chat.model.ChatResponse>> newInstance(org.springframework.ai.chat.prompt.Prompt prompt) {
        return new OCIStreamingConsumer(prompt);
    }

    private final class OCIStreamingConsumer implements java.util.function.Consumer<reactor.core.publisher.FluxSink<org.springframework.ai.chat.model.ChatResponse>> {

        private final org.springframework.ai.chat.prompt.Prompt prompt;

        public OCIStreamingConsumer(org.springframework.ai.chat.prompt.Prompt prompt) {
            this.prompt = prompt;
        }

        @Override
        public void accept(reactor.core.publisher.FluxSink<org.springframework.ai.chat.model.ChatResponse> sink) {
                    ...
                    while ((line = reader.readLine()) != null) {
                        ...
                        if (line.startsWith("data:")) {
                            ...
                            try {
                                ...
                                java.util.Map<String, Object> event = JSON_MAPPER.readValue(data, java.util.Map.class);
     ...
     */
    private static String extractTextDelta(java.util.Map<String, Object> event) {
    ...
    private static Object findFirstText(Object node) {
        if (node instanceof java.util.Map) {
            for (var entry : ((java.util.Map<String, Object>) node).entrySet()) {
